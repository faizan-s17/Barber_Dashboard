...
export default function Clients({ isAdmin }) {
  ...
  async function load() {
    ...
    const [cl, appts] = await Promise.all([
      ...
      supabase.from('appointments').select('customer_phone, start_time, status, booking_id').neq('status', 'cancelled'),
    ])
    setClients(cl.data || [])

    // Build per-phone appointment map. nextRef is the booking reference a caller reads out.
    const map = {}
    for (const a of (appts.data || [])) {
      const ph = a.customer_phone
      if (!ph) continue
      if (!map[ph]) map[ph] = { count: 0, last: null, next: null, nextRef: null }
      map[ph].count++
      if (a.start_time < now) {
        if (!map[ph].last || a.start_time > map[ph].last) map[ph].last = a.start_time
      } else {
        if (!map[ph].next || a.start_time < map[ph].next) {
          map[ph].next = a.start_time
          map[ph].nextRef = a.booking_id
        }
      }
    }
    setApptMap(map)
  ...
  const filtered = clients.filter(c => {
    ...
    if (!q) return true
    // searching by booking reference is the common case: a caller reads it out over the phone
    const ref = apptMap[c.phone]?.nextRef?.toLowerCase() || ''
    return c.name?.toLowerCase().includes(q) || c.phone?.includes(q) ||
           c.email?.toLowerCase().includes(q) || ref.includes(q.replace(/[^a-z0-9]/g, ''))
  })
  ...
  function exportPDF() {
    ...
    w.document.write(`<!DOCTYPE html><html><head><title>Client List</title><style>
      *{margin:0;padding:0;box-sizing:border-box}
      ...
      .header{display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:20px;padding-bottom:12px;border-bottom:2px solid #18181b}
      h1{font-size:20px;font-weight:700;color:#18181b;font-family:Georgia,serif}
      .meta{font-size:11px;color:#666;margin-top:4px}
      table{border-collapse:collapse;width:100%}
      th{background:#18181b;color:#ffffff;text-align:left;padding:8px 10px;font-size:10px;text-transform:uppercase;letter-spacing:.6px}
    ...
    <>
      ...
      <div className="page">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
          <input
            type="text" placeholder="Search name, phone, email or booking ref…"
                ...
                {filtered.map(c => {
                  ...
                  return (
                    <tr key={c.id}>
                      ...
                      <td className="hide-mobile" style={{ fontSize: 12, color: m.next ? 'var(--gold)' : 'var(--text-dim)' }}>
                        {fmtTime(m.next)}
                        {m.nextRef && (
                          <div style={{ fontFamily: 'monospace', fontSize: 11, color: 'var(--text-muted)', letterSpacing: '.5px' }}>
                            {m.nextRef}
                          </div>
                        )}
                      </td>
      ...
      >
        <div className="form-grid cols-1">
          <div className="form-group">
            <label htmlFor="cl-name">Full name <span aria-hidden="true" style={{ color: 'var(--red)' }}>*</span></label>
            <input id="cl-name" type="text" value={form.name} onChange={field('name')}
                   placeholder="e.g. James Smith" required autoComplete="name" />
          </div>
          <div className="form-group">
            <label htmlFor="cl-phone">Phone</label>
            <input id="cl-phone" type="tel" value={form.phone} onChange={field('phone')}
                   placeholder="+44 7700 900000" autoComplete="tel" inputMode="tel" />
            <span className="form-hint">Used for booking texts, so include the country code.</span>
          </div>
          <div className="form-group">
            <label htmlFor="cl-email">Email</label>
            <input id="cl-email" type="email" value={form.email} onChange={field('email')}
                   placeholder="customer@example.com" autoComplete="email" inputMode="email" />
          </div>
          <div className="form-group">
            <label htmlFor="cl-notes">Notes</label>
            <input id="cl-notes" type="text" value={form.notes} onChange={field('notes')}
                   placeholder="e.g. prefers a skin fade, allergic to…" />
          </div>
        </div>
      </Modal>
    </>
