...
const STATUS_COLORS = {
  confirmed:   { bg: 'var(--gold-dim)',  border: 'var(--gold)',  text: 'var(--gold)' },
  rescheduled: { bg: 'var(--amber-bg)',  border: 'var(--amber)', text: 'var(--amber)' },
  cancelled:   { bg: 'var(--red-bg)',    border: 'var(--red)',   text: 'var(--red)' }
}
...
export default function Calendar({ profile }) {
  ...
  useEffect(() => {
    supabase.from('barbers').select('name').eq('active', true).neq('role', 'operator').then(({ data }) => {
      const names = (data || []).map(b => b.name)
      setBarbers(names)
      if (profile?.role !== 'admin' && profile?.name) setSelected(profile.name)
    })
    supabase.from('services').select('name,price,duration_minutes').eq('active', true).then(({ data }) => {
      setServices(data || [])
    })
  }, [profile])
  ...
  function exportCSV() {
    ...
    const headers = ['Date/Time', 'Reference', 'Customer', 'Phone', 'Service', 'Barber', 'Status', 'Notes']
    const rows = appts.map(a => [
      fmtDT(a.start_time), a.booking_id || '', a.customer_name || '', a.customer_phone || '', a.service_name || '', a.barber_name || '', a.status || '', a.notes || ''
  ...
  function exportPDF() {
    ...
    w.document.write(`<!DOCTYPE html><html><head><title>Bookings</title><style>
      *{margin:0;padding:0;box-sizing:border-box}
      body{font-family:Arial,sans-serif;font-size:11px;padding:28px;color:#111}
      .header{margin-bottom:20px;padding-bottom:12px;border-bottom:2px solid #18181b}
    ...
    <div class="header">
      <h1>Barber Shop — Bookings</h1>
      <div class="meta">${rangeLabel} · ${appts.length} booking${appts.length !== 1 ? 's' : ''} · Exported ${new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</div>
    </div>
    <table>
      <thead><tr><th>Date/Time</th><th>Ref</th><th>Customer</th><th>Phone</th><th>Service</th><th>Barber</th><th>Status</th></tr></thead>
      <tbody>${rows}</tbody>
    </table></body></html>`)
    w.document.close()
              ...
              return (
                <button key={i} onClick={() => setSelectedDay(i)}
                  style={{ flex: '1 0 60px', padding: '8px 4px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 12, transition: 'all .15s',
                    background: isSelected ? 'var(--ink)' : isToday ? 'var(--surface3)' : 'var(--surface2)',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, padding: '4px 0' }}>
              ...
              {[
                ...
                <div key={label} style={{ display: 'grid', gridTemplateColumns: '100px 1fr', gap: 8, fontSize: 13 }}>
                  ...
                  <span style={label === 'Reference'
                    ? { color: 'var(--gold)', fontFamily: 'monospace', fontWeight: 700, letterSpacing: '1px' }
...
function NewBookingModal({ barbers, services, defaultBarber, onClose, onSaved }) {
  ...
  function set(key, val) {
    setForm(f => {
      ...
      if (key === 'service_name') {
        ...
        if (svc) { next.service_price = svc.price || ''; next.duration = svc.duration_minutes || 30 }
      }
      return next
    })
  }
  ...
  async function handleSave(e) {
    ...
    if (new Date(startISO).getTime() <= Date.now()) {
      return toast.error('Please pick a future date and time')
    }
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: '4px 0 8px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 100px', gap: 8 }}>
              <div>
                ...
                <input style={inputStyle} type="date" value={form.date} min={todayLondon()}
