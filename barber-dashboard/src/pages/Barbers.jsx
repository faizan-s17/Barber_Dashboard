...
export default function Barbers({ isAdmin }) {
  ...
  async function load() {
    setLoading(true)
    // Operators are SkyWeb staff managing the dashboard, not shop barbers — never list them here.
    const { data } = await supabase.from('barbers').select('*').neq('role', 'operator').order('created_at')
    setBarbers(data || [])
    setLoading(false)
  }
  useEffect(() => { load() }, [])
  ...
  function exportPDF() {
    const rows = barbers.map(b => {
      ...
      // literal hexes: this feeds the print popup, a separate document where CSS vars don't exist
      const statusColor = b.active ? '#047857' : '#71717a'
      return `<tr>
        <td>${esc(b.name)}</td>
        <td>${esc(b.email)}</td>
        <td>${esc(b.role)}</td>
                  ...
                  <tr key={b.id}>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 28, height: 28, background: 'var(--ink)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 12, color: 'var(--on-accent)', flexShrink: 0 }}>
